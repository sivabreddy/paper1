...
- Batch normalization
- Image-to-column transformations
"""
...
def img2col_index(x_shape, k_size, stride=1):
    """
    Generates indices for image-to-column transformation
    Used to efficiently implement convolution operations
    ...
    Returns:
        Tuple of (channel_indices, height_indices, width_indices)
    """
...
class Conv(Layer):
    ...
    Args:
        in_shape: Input shape (channels, height, width)
        k_size: Kernel size
        k_num: Number of kernels
        stride: Convolution stride
...
class MaxPooling(Layer):
    ...
    - Backward pass with gradient routing
    
    Args:
        in_shape: Input shape (channels, height, width)
        k_size: Pooling window size
        stride: Pooling stride (defaults to k_size)
    """
...
class Softmax(Layer):
    ...
    Used as final layer for multi-class classification
    """
    def __init__(self):
...
class Activation(Layer):
    ...
    Args:
        act_type: Activation type ('ReLU', 'Sigmoid', 'Tanh', 'LeakyReLU')
    """
...
class BatchNormalization(Layer):
    ...
    Implements:
    - Normalization over mini-batches
